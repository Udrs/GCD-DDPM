...
class UNetModel(nn.Module):
    ...
    def highway_forward(self,x1,x2, hs):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        h_diff = diff_img.type(self.dtype)
        h = A_img.type(self.dtype)
        h2 = B_img.type(self.dtype)
        # h_diff = h
        c = h[:,:-1,...]
        c2 = h2[:,:-1,...]
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x,x2, hs):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            ...
            x2 = self.conv_blocks_context[d](x2)
            # skips.append(x)
            # skips2.append(x2)
            diff.append(x2-x)
            if not self.convolutional_pooling:
            ...
            if hs:
                ...
                # h2 = self.conv_trans_blocks_a[d](h2)
                h = self.ffparser[d](h)
                # h2 = self.ffparser[d](h2)
                ha = self.conv_trans_blocks_b[d](h)
                # ha2 = self.conv_trans_blocks_b[d](h2)
                hb = th.mean(h,(2,3))
                # hb2 = th.mean(h2,(2,3))
                hb = hb[:,:,None,None]
                # hb2 = hb2[:,:,None,None]
